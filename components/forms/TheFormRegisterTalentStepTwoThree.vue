  		...
  		<div class="container">
					...
					<div class="offset-lg-4 col-lg-16">
						...
						<div class="c-formregistertalent__field">
                        	<label class="--question">{{$t('registerform.steps.two.three.label_question')}}</label>
							<div class="c-formregistertalent__field__radiolist" role="radiogroup">
								<div v-for="(element) in domains" :key="element.id" class="c-formregistertalent__field__radioelement">
									<input type="radio" :id="element.id" :name="element.name" :value="element.id" v-model="specialization" role="radio" aria-checked="false"  :aria-labelledby="'label-' + element.id">
